// DiskArcher.
// CFileToArc class - a file, that the program SHOULD store in the Archive.
...
public:
  ...
  bool Delete();		// (2)
  bool GetStatus();	// (4) Decide, must the file be updated or now
	bool PreCompress();
	CString GetCompressedFileName() const;	// With full path!
	bool IsPreCompressed()	{	return ( m_sCompressedFilename != "" );	}
	void ResetRuntimeData();	// (9) Clear some non-actual runtime information
...
// Only for temporary assignment during updating the Archive. Isn't stored in DB.
  ...
  CFileCopy*	m_pCopyToReplace; // It is only run-time information
