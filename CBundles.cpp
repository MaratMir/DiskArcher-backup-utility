...
CBundle* CBundles::BundleCreate( CRoom *pRoom, bundleType nBundleType/*(3)*/ )
{
	...
	pNewBundle->m_strDir      = pRoom->m_strDir 
							  + ( pRoom->m_strDir.Right(1)=="\\" ? "" : "\\" )
							  + pRoom->m_strName;

// Get a new Bundle ID: it will be maxID+1
	int maxID = g_pTheDB->BundleGetMaxID();
	...
	// Write the Bundle to DB
		bSuccess = g_pTheDB->BundleAdd( pNewBundle );
...
// Delete the Bundle from DB
	if( bSuccess )
		bSuccess = g_pTheDB->BundleDelete( pBundle );
